class Chart
  ...
  def self.game_online(from, to)
    ...
    minutes = []
    ( from.to_i .. to.to_i ).step(1.minute.to_i) { |e| minutes << (e * 1000) }
    game_online = game_online.last( minutes.count )
    data = minutes.zip(game_online)
  ...
  def self.game_online1 # all works
    from = 24.hours.ago
    to   = Time.now

    game_online = Statistic.select(from: from, to: to)
